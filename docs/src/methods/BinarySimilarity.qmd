...
Prepare the data by removing all empty rows and columns.
...
The metaprogramming capabilities of Julia facilitate the implementation of a user-supplied (dis)similarity metric with ease. Only three additional steps are required. First, compose a string representing an equation. Second, create an expression by parsing this string. Last, evaluate this expression.
...
function similarity(;x::Union{Matrix, NamedMatrix}, eq::String)
	samp = Int.(x .!= 0)
	d = samp * transpose(samp)
	s = Array(diag(d))
	n = length(s)
...
similarity(x = mat, eq = "((a .+ b .+ c) ./ ((2 * a .+ b .+ c) ./ 2)) .- 1")
```
...
```{julia macro}
macro binarysimilarity(x::NamedArray, eq::AbstractString)
