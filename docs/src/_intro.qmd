...
```{julia example_data}
x = EcoVeg.generate_test_array(rown = 30, coln = 20, meancoloccs = 10, rowprefix = "SiteA-", colprefix = "Species")
```
...
EcoVeg.print_summary_syntopic_table(syn_2, "normal", "cover_proportion")
```
...
```{julia example_data}
y = EcoVeg.generate_test_array(rown = 5, coln = 30, meancoloccs = 5, rowprefix = "SiteB-", colprefix = "Species")
```
...
syn_y_mat = EcoVeg.extract_syntopic_matrix(syn_y)
syn_1_mat = EcoVeg.extract_syntopic_matrix(syn_1)
syn_2_mat = EcoVeg.extract_syntopic_matrix(syn_2)
```
